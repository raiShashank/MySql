...
#define MAX_SIZE 8 * 1024			//max size of file (excluding d header) in bytes
#define MAX_FRAMES 1000				//dis limits no. of blocks in main memory
...
typedef map<int, tuple > mp;
typedef map<int, tuple >::iterator imp;
...
struct info
{
	...
	blk_size bl_size;
	bool dirty;
	int used_block;			//no. of blocks (including holes) upto which the file has been extended
};
...
class file
{
	...
	queue<int> free_list;		// a list of blocks which will be next allocated . (Implemented such dat holes will b allocated first)
	int max_block;				// no. of blocks(excluding d header) largest file can have
	int max_bytes;				// no. of bytes used by bitmap
	int _offset;				//length of offset in bytes, I need to know whenever file opens
	blk_size block_size;				//block_size in bytes
	mp page_tbl;				
	FILE *fp;
	set <int> dirty_pages;		//stores d list of block numbers which need 2 b written to the file at d time of closing
	set <int> unread_pages;		//stores d list of block numbers which need 2 b removed when memory is insufficient
	...
	inline void set_nth_bit(int idx)			//idex ranges from 0 to MAX_SIZE - 1 
	{
		//cout << idx << "\n";
	    header.bitmap[idx / CHAR_BIT] |= 1 << (idx % CHAR_BIT);
	}

	inline void clear_nth_bit(int idx)
	{
	    header.bitmap[idx / CHAR_BIT] &= ~(1 << (idx % CHAR_BIT));
	}

	inline int get_nth_bit(int idx)
	{
...
public:
	...
	int allocate_block();										//returns d index of newly allocated block
	void *read_block(blk_addr block_address);						//reads starting address of buffer in main memory containg block's contents

	bool write_block(blk_addr block_address, void *buf_addr);		//block at buf_addr (in memory) is written at block_address in dis file
	...
	bool deallocate_block(blk_addr block_address);
	bool unread_block(blk_addr block_address);						//just sets d unread bit to true . when in memory crunch, I can frre dis 
	...
	bool wite_back (blk_addr block_address);							//just sets the dirty bit and d block is still in buffer
};
#endif
